...
globals [
  ...
  black-ratios
  test
  inversion? ; Bei 'true' sind schwarze mit weißen gleichgestellt
]
...
to go
  if done? [
     set white-ratios []
     foreach [ precision (prizes-won / (ticks + 1)) 3  ]  of turtles with [hunch <= 50 ] [
         ; output-print (word "[" ? "]" )
         let next-singleton []
         let next-ratio ?
         set next-singleton fput next-ratio next-singleton
         set white-ratios fput next-singleton white-ratios
     ]
     ...
     foreach [ precision (prizes-won / (ticks + 1)) 3  ]  of turtles with [hunch > 50 ] [
         ...
         let next-ratio ?
         set next-singleton fput next-ratio next-singleton
         set black-ratios fput next-singleton black-ratios
     ]

     csv:to-file "./whites.csv" white-ratios
     csv:to-file "./blacks.csv" black-ratios
     stop
  ]
  ask turtles
  [
  ...
  set pay-off-black mean [prizes-won] of turtles with [hunch > 50 ]
  set ratio-black (mean [ prizes-won / (ticks + 1) ]  of turtles with [hunch > 50 ])
  set pay-off-white mean [prizes-won] of turtles with [hunch <= 50 ]
  set ratio-white (mean [ prizes-won / (ticks + 1) ]  of turtles with [hunch <= 50 ])
  tick
end
...
to award-prizes
  ...
  [
    ...
    [ fd 1 set prizes-won prizes-won + 1]
  ]
...
"" ""
PENS
"Believers" 1.0 0 -14730904 true "" "plot pay-off-black"
"Non-Believers" 1.0 0 -5298144 true "" "plot pay-off-white"

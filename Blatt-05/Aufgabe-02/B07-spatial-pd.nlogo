;;!!! NOCH MODULO_OPERATION EINBAUEN !!!


globals [
  entscheidung1 ;; Speichert Entscheidung des aktuell betrachteten Patches
  entscheidung2 ;; Speichert Entscheidung des Gegners des aktuell betrachteten Patches
  gegner-entscheidung ;; dient als Zwischenablage zur Speicherung der gegnerischen Strategie
  best-strategy ;; Speichert die aktuell beste Strategie eines Nachbarn
  highscore ;; Speichert die aktuell beste Punktzahl eines Nachbarn
]
...
patches-own [
  ...
  scorehist7
  scorehist8
  waiting ;; indicates whether this patch is still waiting to play this tick
  anzahl-waiting ;; counts the neighbored patches that haven't played yet this tick
  zufall ;; Zufallsvariable zur Wahl des Nachbarn
  own-entscheidung ;; Speichert eigene Entscheidung (1 = Cooperate / 2 = Defect)
  enttaeuscht? ;; (Grim Trigger) Ist Patch bereits durch einen nicht kooperierenden Gegner enttäuscht worden?
  verrat-rate ;; (Tranquilizer) aktuelle Rate des Verrats in Prozent
  verrat-plus ;; (Tranquilizer) Gibt an, um wie viel die Verrat-Rate bei gegenseitiger Kooperation ansteigen soll
  erster-verrat ;; (Tranquilizer) Tick, bei dem der Tranquilizer das erste Mal nicht kooperiert
  letzter-verraten? ;; (Tranquilizer) Gibt an, ob beim letzten Mal verraten wurde
  letzte-gegner-strategie ;; (Tit-for-Tat) Gibt die zuletzt vom Gegner gewählte Strategie an
  spiele
]
...
to setup
  ...
  ask patches [
    ...
    set enttaeuscht? false
    set letzter-verraten? false
    set erster-verrat 10
    set verrat-rate 10
    set verrat-plus 3
    set score 0
    set scorehist 0
    set scorehist1 min (list payoff-p payoff-r payoff-s payoff-t)
    set scorehist2 min (list payoff-p payoff-r payoff-s payoff-t)
    set scorehist3 min (list payoff-p payoff-r payoff-s payoff-t)
    set scorehist4 min (list payoff-p payoff-r payoff-s payoff-t)
    set scorehist5 min (list payoff-p payoff-r payoff-s payoff-t)
    set scorehist6 min (list payoff-p payoff-r payoff-s payoff-t)
    set scorehist7 min (list payoff-p payoff-r payoff-s payoff-t)
    set scorehist8 min (list payoff-p payoff-r payoff-s payoff-t)
    set spiele 0
    update-color
  ]
...
to go
  ...
  ask patches [
    interact
    ;;aktualisiere-score
  ]          ;; to play with one neighboring patch
...
to interact  ;; patch procedure
  ask patches with [waiting] [
    triff-entscheidung
    set entscheidung1 own-entscheidung
    set anzahl-waiting (count neighbors with [waiting])

    set zufall (random anzahl-waiting)

    ifelse(zufall = 0) [
      ask patch-at pxcor (pycor - 1) [
        ifelse(waiting = true) [
          set letzte-gegner-strategie gegner-entscheidung
          triff-entscheidung
          set entscheidung2 own-entscheidung
          aktualisiere-gegner-score
          set waiting false
        ][
          set zufall (zufall + 1) ;; Da Zufallszahl nur unter den noch wartenden Patches ermittelt wird, wir aber bei der Fallunterscheidung auf 8 weiterzählen, muss bei jedem bereits gespielten Patch die ermittelte Zufallszahl um 1 erhöht werden
        ]
      ]
    ] [ifelse(zufall = 1) [
      ask patch-at (pxcor + 1) (pycor - 1) [
        ifelse(waiting = true) [
          set letzte-gegner-strategie gegner-entscheidung
          triff-entscheidung
          set entscheidung2 own-entscheidung
  ...
  set letzte-gegner-strategie gegner-entscheidung
  aktualisiere-own-score
  set waiting false
  ]
end

to aktualisiere-own-score
  ifelse(entscheidung1 = 1 AND entscheidung2 = 1) [ ;; Aktualisierung des eigenen Scores
      set score (score + payoff-r)
      set scorehist1 payoff-r
      set scorehist (scorehist1 + scorehist2 + scorehist3 + scorehist4 + scorehist5 + scorehist6 + scorehist7 + scorehist8)
      set verrat-rate (verrat-rate + verrat-plus)
      set letzter-verraten? false
    ][
    ifelse(entscheidung1 = 1 AND entscheidung2 = 2) [
      set score (score + payoff-s)
      set scorehist1 payoff-s
      set scorehist (scorehist1 + scorehist2 + scorehist3 + scorehist4 + scorehist5 + scorehist6 + scorehist7 + scorehist8)
      set enttaeuscht? true
      set letzter-verraten? false
    ][
    ifelse(entscheidung1 = 2 AND entscheidung2 = 1) [
      set score (score + payoff-t)
      set scorehist1 payoff-t
      set scorehist (scorehist1 + scorehist2 + scorehist3 + scorehist4 + scorehist5 + scorehist6 + scorehist7 + scorehist8)
      set letzter-verraten? true
    ][
      set score (score + payoff-p)
      set scorehist1 payoff-p
      set scorehist (scorehist1 + scorehist2 + scorehist3 + scorehist4 + scorehist5 + scorehist6 + scorehist7 + scorehist8)
      set letzter-verraten? true
    ]]]
  aktualisiere-score
...
to aktualisiere-score
  set scorehist8 scorehist7
  set scorehist7 scorehist6
  set scorehist6 scorehist5
  set scorehist5 scorehist4
  set scorehist4 scorehist3
  set scorehist3 scorehist2
  set scorehist2 scorehist1
  set spiele (spiele + 1)
end

to triff-entscheidung
   ifelse(strategy = "random") [
      set gegner-entscheidung "random"
      ifelse(random 100 < 50) [ set own-entscheidung 1 ] [ set own-entscheidung 2 ]
    ] [ifelse(strategy = "always cooperate") [
      set gegner-entscheidung "always cooperate"
      set own-entscheidung 1
    ] [ifelse(strategy = "always defect") [
      set gegner-entscheidung "always defect"
      set own-entscheidung 2
    ] [ifelse(strategy = "grim trigger") [
      set gegner-entscheidung "grim trigger"
      ifelse(enttaeuscht? = false) [ set own-entscheidung 1 ] [ set own-entscheidung 2 ]
    ] [ifelse(strategy = "tranquilizer") [
      set gegner-entscheidung "tranquilizer"
      ifelse(ticks < erster-verrat) [ set own-entscheidung 1 ] [
        ifelse(ticks = erster-verrat) [ set own-entscheidung 2 ] [
          ifelse(letzter-verraten? = true) [ set own-entscheidung 1 ] [
            ifelse(random 100 < verrat-rate) [ set own-entscheidung 2 ] [ set own-entscheidung 1 ]
          ]
        ]
      ]
    ] [set gegner-entscheidung "tit for tat"
       ifelse(ticks = 0) [set own-entscheidung 1] [set own-entscheidung letzte-gegner-strategie ]
    ]
    ]]]]
end

to select-strategy  ;; patch procedure
  set highscore (10 * min (list payoff-t payoff-r payoff-s payoff-t))
  ask neighbors [
    if (scorehist >= highscore) [
      set highscore scorehist
...
"" ""
PENS
"random" 1.0 0 -13345367 true "" "plot count patches with [strategy = \"random\"]"
"cooperate" 1.0 0 -13840069 true "" "plot count patches with [strategy = \"always cooperate\"]"
"defect" 1.0 0 -2674135 true "" "plot count patches with [strategy = \"always defect\"]"
"yellow" 1.0 0 -1184463 true "" "plot count patches with [strategy = \"grim trigger\"]"
"tranquilizer" 1.0 0 -5825686 true "" "plot count patches with [strategy = \"tranquilizer\"]"
"tit for tat" 1.0 0 -955883 true "" "plot count patches with [strategy = \"tit for tat\"]"
"Gesamtzahl" 1.0 0 -16777216 true "" "plot count patches"
